...
export const returnBorrowedBook = catchAsyncErrors(async (req,res,next)=>{
   ...
   await user.save();
    ...
    res.status(200).json({
        success: true,
        message: fine !== 0 ? `The book is returned successfully. The total charges, including a fine, are $${fine+book.price} `:`The book is returned successfully. Fine:$${book.price}`,
        book,
        borrow,
    })
})
